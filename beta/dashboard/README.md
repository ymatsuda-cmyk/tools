...
`chart` を `"bar"` にすると、リングの代わりに棒グラフが出ます。数える条件（`statuses` / `statusMatch`）は
リングと同じです。棒の並びは選んでいるパターンで変わります。

- **すべて・動画・Web記事**（分類をまたぐパターン）… **分類別**に1本ずつ並べます。分類が空のものは「未分類」です
- **other / music / golf**（分類で絞るパターン）… 棒が1本になってしまうので、代わりに**状態別**に並べます

棒の長さは一番多いものを基準にし、色はその分類の中での溜まり具合（黄色→赤）で変わります。
0件のものは並びません。
