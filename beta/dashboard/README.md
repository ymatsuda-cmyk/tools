...
| `chart` | 件数の見せ方。`"ring"`（既定）か `"bar"`（分類別の棒グラフ） |
...
`chart` を `"bar"` にすると、リングの代わりに**分類別の棒グラフ**が出ます。数える条件
（`statuses` / `statusMatch`）はリングと同じで、それを分類ごとに1本ずつ並べます。
分類が空のものは「未分類」にまとめます。分類で絞るパターン（other / music / golf）を選んでいるときは
その分類の1本だけになります。
