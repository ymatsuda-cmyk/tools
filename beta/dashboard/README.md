...
行に名前を付けると、他の行の式からその名前で参照できます。名前を使った式は、
数字に開いた形で1行下に出て、**元の名前がルビで数字の上に乗ります**。

```
税込   単価*個数*1.1
       1,200 × 3 × 1.1     ← 1,200 の上に「単価」、3 の上に「個数」
                   3,960
```

名前は式の欄の上に並ぶボタンからも入れられます。行の並び順は関係なく、
下の行を上の行から参照しても構いません。参照が輪になっている場合は
「◯◯が循環しています」と出て止まります。

#### 入力

- 名前・式の欄はどちらもそのまま編集できます
- テンキー（カード下のボタン）と変数ボタンは、最後に触れた行のカーソル位置に入ります。
  物理キーボードのテンキーでも入力できます
- 結果は入力しながら出ます。`=` はありません
