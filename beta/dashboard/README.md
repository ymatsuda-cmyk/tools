...
待機中のリングは、状態が `完了`（＝文字起こしは済んだがAI生成がまだ）のものが
`total` に対して何件たまっているかを出します。溜まるほど黄色→赤に変わります。
実行中はリングが進捗に切り替わり、`処理済み/対象` と、生成中の段・動画タイトルが出ます。
...
対象の件数は cron が書き出した静的JSON（`data/clipstock/index-*.json`）から数えます。
Notion 側で状態を変えた直後は、次の書き出しまで件数が追いつきません。
